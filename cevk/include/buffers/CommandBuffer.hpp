...
namespace ce {
    ...
    class CommandBuffer {
      public:
        ...
        void submitToRender(VkQueue gQueue, VkSemaphore wait, VkSemaphore signal, VkFence fence, VkPipelineStageFlagBits pipelineStageFlags,
                            size_t bufferIndex) {
            // -- SUBMIT COMMAND BUFFER TO RENDER
            // Queue submission information
            std::array<VkSemaphore, 1> waitSemaphores{wait};                    //{this->imageAvailable[this->currentFrame]};
            std::array<VkSemaphore, 1> signalSemaphores{signal};                //{this->renderFinished[this->currentFrame]};
            std::array<VkPipelineStageFlags, 1> waitStages{pipelineStageFlags}; //{VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT};
            ...
            const VkSubmitInfo submitInfo{
                ...
                .pCommandBuffers = &this->commandBuffers[bufferIndex], // Command buffer to submit
            ...
            if (vkQueueSubmit(gQueue, 1, &submitInfo, fence) != VK_SUCCESS) {
